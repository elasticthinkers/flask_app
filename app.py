...
def generate_fix_queries_with_bedrock(input_json, primary_key ,target_table , strategy, primary_key_type):
    print("✏️ Generating SQL fix queries using Bedrock...",target_table)
    prompt = f"""
    ...
    In query keep table name as {target_table} and do not add source_{target_table}
    Use this format:
    [
      {{
    ...
    Example query:
    UPDATE {target_table} 
    SET name = (SELECT name FROM {target_table} WHERE {primary_key} = 5) 
    WHERE {primary_key} = 5;
    📊 Mismatch JSON:
    {json.dumps(input_json, indent=2)}
    ...
    In Table name do not include source_ or table_ keep only {target_table}.
...
def remediate():
    ...
    try:
        ...
        # 3. Row Data Mismatch
            ...
            queries = generate_fix_queries_with_bedrock(body,primary_key,target_table, strategy, primary_key_type)
